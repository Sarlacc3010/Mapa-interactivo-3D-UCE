...
export function EventsPopup({
  ...
  // ===========================================================================
  // 1. LÓGICA DE FECHAS (ROBUSTA)
  // ===========================================================================

  // Helper: Convierte cualquier input de fecha a string "YYYY-MM-DD"
  // Funciona con Date objects, strings ISO, strings SQL, etc.
  const normalizeDate = (dateInput) => {
    if (!dateInput) return "";

    // Si ya es un objeto Date
    if (dateInput instanceof Date) {
      // Ajustamos al uso horario local para no perder el día por diferencia horaria
      const offset = dateInput.getTimezoneOffset() * 60000;
      const localDate = new Date(dateInput.getTime() - offset);
      return localDate.toISOString().split("T")[0];
    }

    // Si es un string (ej: "2026-02-01 00:00:00" o "2026-02-01T...")
    // Tomamos simplemente los primeros 10 caracteres que son la fecha
    return String(dateInput).substring(0, 10);
  };

  const todayString = normalizeDate(new Date()); // Fecha de hoy local "YYYY-MM-DD"

  // Helper visual para separar Mes y Día
  const getDateParts = (dateString) => {
    if (!dateString) return { month: "---", day: "--" };
    try {
      const cleanDate = normalizeDate(dateString);
      const parts = cleanDate.split("-"); // [2026, 02, 01]

      // Creamos fecha usando constructor local (Año, Mes-1, Día)
  ...
  const localEvents = events
    .filter((e) => {
      // Filtro 1: Coincidencia de Lugar
      const isSameLocation =
        e.location_id &&
        locationId &&
        String(e.location_id) === String(locationId);

      // Filtro 2: Coincidencia de Fecha (Futuro o Hoy)
      const eventDateStr = normalizeDate(e.date);
      const isFutureOrToday = eventDateStr >= todayString;

      return isSameLocation && isFutureOrToday;
    })
    .sort((a, b) => {
      // Ordenar del más próximo al más lejano
    ...
    return (
      <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
        ...
        <div
          ...
          className="relative w-full max-w-lg bg-slate-900/95 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-8 duration-300 flex flex-col max-h-[85vh] border border-white/10 ring-1 ring-cyan-500/20"
        >
          {/* Header Neón */}
          <div className="relative p-6 shrink-0 border-b border-white/10 overflow-hidden bg-slate-950">
            ...
            <div className="relative z-10 flex justify-between items-start">
              <div>
                ...
                <h2 className="text-2xl font-bold leading-tight text-white tracking-tight drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">
                  {locationName}
                </h2>
                <div className="flex items-center gap-1.5 text-slate-400 text-xs mt-2 font-mono">
                  <MapPin size={12} className="text-pink-500" /> Campus
                  Universitario
                </div>
              </div>
              <button
                onClick={onClose}
                className="group p-2 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white rounded-full transition-all duration-300 border border-white/5 hover:border-white/20"
              >
                <X
                  size={20}
            ...
            {localEvents.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full min-h-[200px] text-center opacity-60">
                ...
                <p className="text-xs text-slate-500 mt-2">
                  No hay eventos programados para hoy o el futuro.
                </p>
              </div>
            ) : (
                  ...
                  return (
                    <div
                      ...
                      className={`group relative flex gap-4 p-4 rounded-2xl transition-all duration-300 cursor-default border 
                                   ${
                                     isToday
                                       ? "bg-cyan-950/40 border-cyan-500/50 shadow-[0_0_15px_rgba(6,182,212,0.15)]" // Estilo HOY
                                       : "bg-slate-800/50 border-white/5 hover:bg-slate-800 hover:border-cyan-500/30 hover:shadow-[0_0_20px_rgba(6,182,212,0.1)]" // Estilo Futuro
                                   }`}
                    >
                      {/* Indicador lateral */}
                      <div
                        className={`absolute left-0 top-4 bottom-4 w-1 rounded-r-full transition-opacity shadow-[0_0_10px_#06b6d4]
                                      ${isToday ? "bg-cyan-400 opacity-100" : "bg-cyan-600 opacity-0 group-hover:opacity-100"}`}
                      ></div>

                      <div className="flex flex-col items-center shrink-0">
                        <div
                          className={`h-6 w-12 text-[9px] font-bold uppercase tracking-wider flex items-center justify-center rounded-t-lg border-t border-x border-white/5 transition-colors
                                          ${isToday ? "bg-cyan-600 text-white" : "bg-slate-700 text-slate-300 group-hover:bg-cyan-900/50 group-hover:text-cyan-300"}`}
                        >
                          {month}
                        </div>
                        <div
                          className={`h-10 w-12 text-lg font-bold flex items-center justify-center rounded-b-lg border border-white/5 transition-colors shadow-inner
                      ...
                      <div className="flex-1 min-w-0 py-0.5 pl-2">
                        <div className="flex justify-between items-start gap-2">
                          <h4
                            className={`font-bold text-sm leading-snug transition-colors mb-2 ${isToday ? "text-cyan-300" : "text-slate-200 group-hover:text-cyan-400"}`}
                          >
                            {event.title}
                          </h4>
                          {isToday && (
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          <span
                            className={`flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide border 
                                              ${isToday ? "bg-cyan-500/20 text-cyan-200 border-cyan-500/30" : "bg-purple-500/10 text-purple-300 border-purple-500/30"}`}
                          >
                            <Clock size={10} /> {event.time || "Todo el día"}
                          </span>
                        </div>
  ...
  // ===========================================================================
  // 4. RENDERIZADO (MODO CLARO - INSTITUCIONAL)
  // ===========================================================================
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
      <div
        ...
        <div className="p-5 overflow-y-auto custom-scrollbar flex-1 bg-gray-50 relative">
          {localEvents.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full min-h-[200px] text-center opacity-70">
              ...
              <h3 className="text-[#1e3a8a] font-bold text-base">
                Sin actividades próximas
              </h3>
                ...
                return (
                  <div
                    ...
                    className={`group relative flex gap-4 p-4 rounded-2xl transition-all duration-300 cursor-default border
                                 ${
                                   isToday
                                     ? "bg-blue-50/80 border-blue-200 shadow-md ring-1 ring-blue-100" // Estilo HOY
                                     : "bg-white border-gray-200/60 shadow-sm hover:shadow-lg hover:shadow-blue-900/5 hover:-translate-y-0.5" // Estilo Futuro
                                 }`}
                  >
                    {/* Indicador lateral */}
                    <div
                      className={`absolute left-0 top-4 bottom-4 w-1 rounded-r-full transition-opacity
                                    ${isToday ? "bg-[#D9232D] opacity-100" : "bg-[#1e3a8a] opacity-0 group-hover:opacity-100"}`}
                    ></div>

                    <div className="flex flex-col items-center shrink-0">
                      <div
                        className={`h-6 w-12 text-[9px] font-bold uppercase tracking-wider flex items-center justify-center rounded-t-lg shadow-sm z-10 transition-colors
                                       ${isToday ? "bg-[#D9232D] text-white" : "bg-gray-200 text-gray-600 group-hover:bg-[#D9232D] group-hover:text-white"}`}
                      >
                        {month}
                      </div>
                      <div
                        className={`h-10 w-12 text-lg font-bold flex items-center justify-center rounded-b-lg border-x border-b border-gray-100 shadow-sm transition-colors
                    ...
                    <div className="flex-1 min-w-0 py-0.5 pl-1">
                      <div className="flex justify-between items-start gap-2">
                        <h4
                          className={`font-bold text-sm leading-snug transition-colors mb-1.5 ${isToday ? "text-[#D9232D]" : "text-gray-900 group-hover:text-[#1e3a8a]"}`}
                        >
                          {event.title}
                        </h4>
                        {isToday && (
