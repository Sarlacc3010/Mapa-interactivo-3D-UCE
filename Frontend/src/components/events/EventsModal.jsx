...
export function EventsModal({ isOpen, onClose, location }) {
  ...
  useEffect(() => {
    if (isOpen && location) {
      ...
      // 2. Cargar Mis Suscripciones (Para pintar los botones)
      // Importante: 'credentials: include' envía la cookie de sesión automáticamente
      const fetchSubs = fetch(`${API_BASE}/api/calendar/my-subscriptions`, { credentials: 'include' })
            ...
            <div className="relative z-10 flex justify-between items-start">
              <div>
                ...
                <div className="flex items-center gap-1.5 text-slate-400 text-xs mt-2 font-mono">
                  <MapPin size={12} className="text-pink-500" /> Campus Universitario
                </div>
              </div>
              <button
                onClick={onClose}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 py-0.5 pl-2 flex flex-col h-full">
                        ...
                        {/* 🔥 BOTÓN AGENDAR (DARK) - CON LÓGICA ACTIVA */}
                        <div className="mt-auto flex justify-end">
                            <button 
                                onClick={() => handleToggleEvent(event.id)}
                                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all
                                   ${isSaved 
                                       ? "bg-cyan-500 text-black border-cyan-400 shadow-[0_0_10px_rgba(6,182,212,0.5)]" // Estilo Guardado
                                       : "bg-cyan-950 text-cyan-400 border border-cyan-500/30 hover:bg-cyan-900 hover:text-white hover:border-cyan-400/50 hover:shadow-[0_0_10px_rgba(6,182,212,0.3)]" // Estilo Normal
                                   }`}
                            >
